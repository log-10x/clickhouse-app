...
-- six SQL functions, and two views (tenx.events and tenx.events_iso).
...
-- ---------------------------------------------------------------------------
-- 8. Two views. Both expose the same column shape; pick by what your
--    downstream consumers expect.
--
--    tenx.events       - preserves the original timestamp format per template
--                         (multiIf dispatch over 17 format patterns)
--    tenx.events_iso   - normalises all timestamps to ISO 8601 with ms
--                         precision (single constant format, fastest path)
-- ---------------------------------------------------------------------------
...
DROP VIEW IF EXISTS tenx.events_iso;
CREATE VIEW tenx.events_iso AS
SELECT
    container, namespace, pod, templateHash,
    log AS encoded_log,
    tenx_inflate_iso(
