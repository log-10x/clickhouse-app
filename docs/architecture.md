...
This document covers the decode flow, the design choices behind the ClickHouse port, and the measured performance characteristics that back the value-prop claims in the [README](../README.md).
...
1. Decode Log10x-encoded events transparently at query time
...
| Splunk encoded events index | `tenx.encoded_events` MergeTree | Standard ClickHouse columnar storage with materialized columns for filter pushdown |
...
An earlier iteration of this port shipped a Rust executable UDF alongside the SQL implementation, on the assumption that a compiled binary would outperform pure SQL. Benchmarks proved the opposite: the SQL ISO-only path decodes the full 137K-row sample in ~60 ms versus the Rust UDF's 5.8 s, because the SQL path stays inside ClickHouse's vectorized execution engine while the Rust path pays per-row stdin/stdout IPC.
...
One constant, fully vectorized. **All timestamps render as ISO 8601** regardless of original template. No per-row dispatch. Exposed as `tenx.events_iso`.

Both views ship by default; the customer queries whichever matches their downstream consumer's expectations.

## Measured performance

All measurements taken on the included demo data (200 MB raw OpenTelemetry log, 130 MB encoded, 1.2 MB templates) on an 8-core Apple Silicon Mac running ClickHouse 26.5 in Docker.
...
| `enc_lz4` (encoded logs, LZ4) | 137,418 | 6.23 MiB | 21x |
| `raw_zstd` (original logs, ZSTD-3) | 197,430 | 3.37 MiB | 64x |
| `enc_zstd` (encoded logs, ZSTD-3) | 137,418 | 2.45 MiB | 55.2x |
...
- ZSTD: ~0% (encoded slightly *larger* per row; ZSTD's pattern detection overlaps the templating layer)
...
35% fewer wire bytes (200 MB raw → 130 MB encoded), independent of codec. This is the most defensible compression number because it applies to network and ingest before ClickHouse's compression runs.

## Codec interaction insight

The on-disk measurements show a non-obvious result: **ZSTD's pattern detection overlaps significantly with the templating layer's pattern extraction**. After templating removes the structural redundancy in log text, ZSTD has less to work with. The encoded form has a marginally lower per-byte compression ratio than the raw form.
...
Query-hygiene rule for users: **always filter on the materialized columns first**, then let the surviving rows decode.
...
-- SLOW: filter operates after decode, every row gets inflated
SELECT decoded_log FROM tenx.events
...
| Materialize the decoded column at INSERT time | Defeats the storage savings; decoded form is 3x larger than encoded |
