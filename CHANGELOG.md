...
- **Two views over the same encoded data**:
...
- **Materialized columns** on the encoded events table for fast filter pushdown on `container`, `templateHash`, `namespace`, `pod`, and the encoded `log` payload
- Verified end-to-end on a 200 MB OpenTelemetry-demo sample (3,473 templates, 137,418 encoded events)
- Measured on-disk reduction vs raw events under identical compression: 42% with LZ4, 27% with ZSTD
